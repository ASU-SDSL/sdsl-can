...
```powershell
west build -p always -b nucleo_f103rb tests\eps_message_test -- "-DCONF_FILE=prj.conf;node_a.conf"
west flash
```
...
```powershell
west build -p always -b nucleo_f103rb tests\eps_message_test -- "-DCONF_FILE=prj.conf;node_b.conf"
west flash
```
