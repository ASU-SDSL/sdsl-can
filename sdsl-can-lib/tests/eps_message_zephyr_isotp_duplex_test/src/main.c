...
LOG_MODULE_REGISTER(eps_zephyr_isotp_duplex, LOG_LEVEL_INF);

#ifndef CONFIG_CAN_LINK_TX_PERIOD_MS
#define CONFIG_CAN_LINK_TX_PERIOD_MS 1500
#endif

#ifndef CONFIG_CAN_LINK_TX_START_OFFSET_MS
#define CONFIG_CAN_LINK_TX_START_OFFSET_MS 0
#endif
               ...
               size_t *encoded_len)
{
    ...
    /*
     * Keep the encoded protobuf safely above the single-frame limit so this
     * test exercises multi-frame ISO-TP in both directions.
     */
...
static size_t build_tx_plan(struct tx_action *plan, size_t max_actions)
{
    if (max_actions == 0U) {
        return 0U;
    }
...
int main(void)
{
    ...
    plan_len = build_tx_plan(tx_plan, ARRAY_SIZE(tx_plan));

    LOG_INF("Zephyr ISO-TP duplex test node %u online. TX Period: %dms offset=%dms",
        (unsigned int)local_node, CONFIG_CAN_LINK_TX_PERIOD_MS,
        CONFIG_CAN_LINK_TX_START_OFFSET_MS);

    if (CONFIG_CAN_LINK_TX_START_OFFSET_MS > 0) {
        k_msleep(CONFIG_CAN_LINK_TX_START_OFFSET_MS);
    }
